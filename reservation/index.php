<?php
    ...
    $sql = "SELECT s.*, 
                   ...
                   (SELECT COUNT(*) FROM plot p WHERE p.section_id = s.section_id) AS total_plots
            FROM section s";
    ...
    <?php 
      if($result->num_rows > 0){
        while($row = mysqli_fetch_array($result)){
          $isOccupied = $row['occupied_plots'] >= $row['total_plots']; // Check if all plots are occupied
          ...
          // Check if section is fully occupied
          if ($isOccupied) {
            echo "<div class='alert alert-danger text-center p-1 fw-bold' role='alert'>Already Occupied</div>";
          } else {
